...
public class UIScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		else
		{
			if (gameType == GameType.Score)
			{
				...
				//numberLabels[0].text = numberLabels[1].text = Scoring.totalScore.toString();
				//scores[0] = scores[1] = 0;
			}
			else
			{
		...
		{
			scores[0] = GameManager.Instance.totalScore;
			scores[1] = GameManager.Instance.totalScore;
	...
	public void AddPoints(int playerNumber, int amount = 1)
	{
		...
		UpdateScore(scores[playerNumber]);

		if (numberOfPlayers == Players.OnePlayer)
		{

			numberLabels[1].text = GameManager.Instance.totalScore.ToString(); //with one player, the score is on the right
