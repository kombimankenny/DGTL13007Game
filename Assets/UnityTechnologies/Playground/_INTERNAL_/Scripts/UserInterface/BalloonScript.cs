...
public class BalloonScript : MonoBehaviour
{
    ...
    public void Setup(string dialogueString, bool _isUsingButton, KeyCode _buttonUsed, float _time, Color backgroundC, Color textC, Transform _targetObj = null)
    {
        ...
        if (targetObj == null)
        {
            ...
            rectTransform.position = RectTransformUtility.WorldToScreenPoint(UnityEngine.Camera.main, Vector3.zero);
        }
        else
        {
            rectTransform.pivot = new Vector2(0.5f, 0f); //pivot is at the bottom
            FollowTarget(UnityEngine.Camera.main);
        }
    }
    ...
    private void FollowTarget(UnityEngine.Camera camera)
    {
        ...
        rectTransform.position = RectTransformUtility.WorldToScreenPoint(camera, topBoundary);
    }
}
